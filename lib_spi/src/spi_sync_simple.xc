...
void get_mode_bits(spi_mode_t mode, unsigned &cpol, unsigned &cpha){
    switch(mode){
...
void spi_master_single_client(server interface spi_master_if i,
    ...
    while(1){
        select {
            case accepting_new_transactions => i.begin_transaction(unsigned device_index,
            ...
            case i.end_transaction(unsigned ss_deassert_time):{
            ...
            case i.transfer8(uint8_t data)-> uint8_t r :{
                if(isnull(cb)) {
            ...
            case i.transfer32(uint32_t data) -> uint32_t r:{
                if(isnull(cb)) {
