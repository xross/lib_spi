...
#include "spi_xcmm_transport.h" // api.h
#include "spi_gen.inc.c"
...
static inline void spi_begin_transaction_impl(spi_ctx_t *ctx, unsigned device_index, unsigned speed_in_khz, spi_mode_t mode)
{
...
__attribute__((always_inline))
static void spi_server_impl(const struct rxc_server_vt* vt, spi_server_t srv,
        size_t num_clients,
        port_t p_sclk,
        port_t p_mosi,
        port_t p_miso,
        ...
        const size_t num_slaves)
{
    ...
    while (1)
    {
        rxc_request_context_t rctx = rxc_configure_and_await_one(vt, &srv->s);
        switch (spi_valid_tag(rctx))
        {
            case spi_tag_begin_transaction:
            {

                unsigned device_index = 0;
                unsigned  speed_in_khz = 0;
                spi_mode_t mode;
                ...
                spi_get_args_begin_transaction(rctx, &device_index, &speed_in_khz, &mode);
                ...
                spi_set_response_begin_transaction(rctx);
                break;
            }

            case spi_tag_end_transaction:
            {
                unsigned ss_deassert_time;

                spi_get_args_end_transaction(rctx, &ss_deassert_time);
                ...
                spi_set_response_end_transaction(rctx);
            ...
            case spi_tag_transfer8:
            {
                ...
                spi_get_args_transfer8(rctx, &wrdata);
                ...
                spi_set_response_transfer8(rctx, rddata);
                break;
            }
            ...
            case spi_tag_transfer32:
            {
                ...
                spi_get_args_transfer32(rctx, &wrdata);
                ...
                spi_set_response_transfer32(rctx, rddata);
                break;
            }

            case spi_tag_get:
                spi_set_response_get(rctx, state);
                break;
            case spi_tag_set:
                spi_get_args_set(rctx, &state);
                spi_set_response_set(rctx);
                break;
            case spi_tag_triple:
                state *= 3;
                spi_set_response_triple(rctx);
                break;
            case spi_tag_add_all:
            {
                unsigned a0;
                short a1;
                char a2;
                spi_get_args_add_all(rctx, &a0, &a1, &a2);
                state += a0 + a1 + a2;
                spi_set_response_add_all(rctx);
                break;
            }
            case spi_tag_stop:
                spi_set_response_stop(rctx);
                return;
            default:
...
#if NUM_CLIENTS == 1
        size_t idx;
        //printstr("server waiting for a request\n");
        rxc_request_context_t rctx = rxc_configure_and_await_one(vt, &srv->s);
        printstr("server: got request from client\n");
#else
        rxc_request_handle_t req = vt->complete_and_await(&servers, &c);
#endif
        int x = spi_valid_tag(rctx);
...
#if NUM_CLIENTS == 1
void spi_server_remote(const spi_server_args_t* args,  spi_server_params_t* params)
{
    spi_server_impl(&rxc_transport_remote_shared.svt, args->srv
